"""Tool implementations for Meta Campaign operations."""

from __future__ import annotations

from typing import Any

from meta_ads_mcp.api.client import MetaAPIClient
from meta_ads_mcp.api.pagination import extract_pagination_info
from meta_ads_mcp.security import validate_id

DEFAULT_CAMPAIGN_FIELDS: list[str] = [
    "name",
    "status",
    "effective_status",
    "objective",
    "buying_type",
    "daily_budget",
    "lifetime_budget",
    "budget_remaining",
    "created_time",
    "start_time",
    "stop_time",
]

DEFAULT_INSIGHTS_FIELDS: list[str] = [
    "campaign_name",
    "impressions",
    "clicks",
    "spend",
    "ctr",
    "cpc",
    "cpm",
    "reach",
    "frequency",
    "actions",
    "cost_per_action_type",
    "date_start",
    "date_stop",
]


def list_campaigns(
    client: MetaAPIClient,
    account_id: str,
    fields: list[str] | None = None,
    effective_status: list[str] | None = None,
    limit: int = 25,
) -> dict[str, Any]:
    """List campaigns for an ad account.

    Args:
        client: Meta API client.
        account_id: Ad account ID (e.g., 'act_123456789').
        fields: Fields to retrieve. Defaults to standard campaign fields.
        effective_status: Filter by status (e.g., ['ACTIVE', 'PAUSED']).
        limit: Maximum number of campaigns to return (default: 25).
    """
    validate_id(account_id, "account_id")

    params: dict[str, Any] = {
        "fields": fields or DEFAULT_CAMPAIGN_FIELDS,
        "limit": limit,
    }
    if effective_status is not None:
        params["effective_status"] = effective_status

    response = client.get_edge(account_id, "campaigns", **params)

    return {
        "data": response.get("data", []),
        "pagination": extract_pagination_info(response),
    }


def get_campaign(
    client: MetaAPIClient,
    campaign_id: str,
    fields: list[str] | None = None,
) -> dict[str, Any]:
    """Get details of a specific campaign.

    Args:
        client: Meta API client.
        campaign_id: Campaign ID.
        fields: Fields to retrieve. Defaults to standard campaign fields.
    """
    validate_id(campaign_id, "campaign_id")

    response = client.get_node(
        campaign_id,
        fields=fields or DEFAULT_CAMPAIGN_FIELDS,
    )

    return {"data": response}


def get_campaign_insights(
    client: MetaAPIClient,
    campaign_id: str,
    fields: list[str] | None = None,
    date_preset: str | None = None,
    time_range: dict[str, str] | None = None,
    time_increment: str | None = None,
    breakdowns: list[str] | None = None,
    level: str | None = None,
    limit: int = 25,
) -> dict[str, Any]:
    """Get performance insights for a specific campaign.

    Args:
        client: Meta API client.
        campaign_id: Campaign ID.
        fields: Metrics to retrieve. Defaults to standard insights fields.
        date_preset: Date range preset (today, yesterday, last_7d, last_30d, etc.).
        time_range: Custom date range {'since': 'YYYY-MM-DD', 'until': 'YYYY-MM-DD'}.
        time_increment: Time granularity (1, 7, 28, monthly, all_days).
        breakdowns: Breakdown dimensions (e.g., ['age', 'gender']).
        level: Aggregation level (ad, adset, campaign, account).
        limit: Maximum number of results (default: 25).
    """
    validate_id(campaign_id, "campaign_id")

    params: dict[str, Any] = {
        "fields": fields or DEFAULT_INSIGHTS_FIELDS,
        "limit": limit,
    }
    if date_preset is not None:
        params["date_preset"] = date_preset
    if time_range is not None:
        params["time_range"] = time_range
    if time_increment is not None:
        params["time_increment"] = time_increment
    if breakdowns is not None:
        params["breakdowns"] = breakdowns
    if level is not None:
        params["level"] = level

    response = client.get_edge(campaign_id, "insights", **params)

    return {
        "data": response.get("data", []),
        "pagination": extract_pagination_info(response),
        "summary": response.get("summary"),
    }
